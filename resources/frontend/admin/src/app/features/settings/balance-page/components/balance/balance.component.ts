...
export class BalanceComponent {
  ...
  getPending(balance: any) {
    let total = 0;

    if (!balance?.pending_owing && !balance?.pending_owed) {
      return 0
    }

    if (balance?.pending_owing) {
      total -= balance.pending_owing
    }

    if (balance?.pending_owed) {
      total += balance.pending_owed
    }
