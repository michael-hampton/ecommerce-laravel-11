...
const defaultState: FilterState<Question> = {
  data: {} as PagedData<Question>,
  filter: {...defaultPaging, ...{sortBy: 'question'}}
};

@Injectable()
export class SupportQuestionStore extends FilterStore<Question> {
  ...
  loadData = this.effect((filter$: Observable<FilterModel>) =>
    filter$.pipe(
      ...
      switchMap((filter: FilterModel) => this._api.getData(filter).pipe(
          tapResponse({
            next: (data) => this.patchState({data: data as PagedData<Question>}),
