        ...
        <div class="card py-2 mb-3">
            <div class="card-body">
                <div class="d-flex justify-content-between align-items-center">
                    ...
                    <a href="{{route('admin.categories.create')}}" class="btn btn-primary add">Add New Category</a>
                </div>
            </div>
        </div>
                    ...
                    {data: 'id', name: 'id'},
                    {
                        data: 'name', name: 'name', className: 'd-flex', render: function (data, type, row) {
                            return '<div class="d-flex align-items-center justify-content-between me-3">' +
                                '<img src="{{asset('images/categories')}}/'+row.image+'" alt="'+row.name+'" class="image"></div>' +
                                '<div class="">' +
                        ...
                        }},
                    {
                        ...
                        render: function (data, type, row) {
                            return '<div class="d-flex align-items-center justify-content-between">' +
                                '<a data-url="{{route('admin.categories.edit', ['id' => 'test'])}}" data-id=' + row.id + ' href="#" target="_blank" class="edit">' +
                                '<i class="icon-eye"></i>' +
                                '</a>' +
                                '<a data-url="{{route('admin.categories.destroy', ['id' => 'test'])}}" data-id=' + row.id + ' href="#" target="_blank" class="delete">' +
                                '<i class="fa fa-trash"></i>' +
                                '</a>' +
                                '</div>';
                        }
